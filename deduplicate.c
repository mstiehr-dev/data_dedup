...
// BEGINN DER VERARBEITUNG
	...
	for(bytesBufferedTotal = 0L; bytesBufferedTotal<inputFileLen; bytesBufferedTotal+=bytesActuallyBuffered) {
		...
		bytesActuallyBuffered =fread(inputFileBuffer,1,bytesBufferSize,inputFile); // liest 1*bytesBufferSize aus inputFile nach inputFileBuffer (Rückgabewert ist Anzahl der gelesenen Bytes, wenn size 1 ist ) // bytesActuallyBuffered = bytesBufferSize > (inputFileLen-bytesBufferedTotal) ? inputFileLen - bytesBufferedTotal : bytesBufferSize; 
...
// #### HASH SUCHE 
			...
			*hashInJournalPos = isHashInMappedJournal(md5String, journalMapAdd, journalEntries);
			...
			CUDA_HANDLE_ERR( cudaMemcpyToSymbol(entries, &journalEntries, sizeof(journalEntries)) ); 
